#[macro_use]
extern crate rocket;
use detective::data::suspect::{Location, LocationArgs, Mood, Position, Suspect, SuspectArgs};
use flatbuffers;
...
fn info() -> Vec<u8> {
    ...
    let age: u32 = 59;
    let face_img = builder.create_string(
    ...
    let location_prev1 = Location::create(
        &mut builder,
        &LocationArgs {
            pos: Some(&pos),
            street: Some(street),
    ...
    let location_prev2 = Location::create(
        &mut builder,
        &LocationArgs {
            pos: Some(&pos),
            street: Some(street),
